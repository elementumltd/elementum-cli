...
Examples:
  ...
  ei tables create --config table.yaml --output my-table.tf
...
func init() {
	...
	tablesCreateCmd.Flags().String("config", "", "YAML config file path")
	tablesCreateCmd.Flags().StringP("output", "o", "", "Output file path (default: stdout)")
	tablesCreateCmd.Flags().Bool("curl", false, "Output curl command instead of Terraform HCL")
	tablesCreateCmd.Flags().Bool("json", false, "Output resolved config as JSON (for debugging)")
}
...
func runTablesList(cmd *cobra.Command, args []string) error {
	...
	// Create table
	table := ui.NewTable([]string{"NAME", "HANDLE", "TYPE", "CLOUDLINK", "ID"})
...
func runTablesCreate(cmd *cobra.Command, args []string) error {
	ctx := context.Background()
